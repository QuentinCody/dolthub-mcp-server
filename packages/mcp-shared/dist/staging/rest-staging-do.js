...
import { detectArrays, inferSchema, materializeSchema, computeColumnProfiles, } from "./schema-inference";
...
export class RestStagingDO extends DurableObject {
    ...
    persistColumnProfiles(schema) {
        try {
            ...
            this.ctx.storage.sql.exec(`CREATE TABLE IF NOT EXISTS _column_profiles (
					id INTEGER PRIMARY KEY,
					profiles_json TEXT
				)`);
            this.ctx.storage.sql.exec(`INSERT OR REPLACE INTO _column_profiles (id, profiles_json) VALUES (1, ?)`, JSON.stringify(profiles));
        }
        catch {
